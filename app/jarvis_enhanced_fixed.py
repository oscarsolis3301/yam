...
class EnhancedJarvisTrainer:
    ...
    def load_existing_hashes(self):
        ...
        try:
            self.cursor.execute("SELECT question, answer FROM chat_qa")
            existing_pairs = self.cursor.fetchall()
    ...
    def insert_qa_batch(self, qa_pairs: List[Tuple[str, str, str]]) -> int:
        ...
        try:
            ...
            self.cursor.executemany(insert_sql, unique_pairs)
            self.conn.commit()
...
def main():
    ...
    # Initialize trainer
    trainer = EnhancedJarvisTrainer(args.db_path)
